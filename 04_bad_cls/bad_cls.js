...
async function fakeProductFetch() {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(productList);
        }, 2000); // Longer delay to show loading and layout shift
    });
}
...
function loadProducts() {
    ...
    // Show loading indicator and clear products
    loadingIndicator.style.display = "block";
    productListContainer1.innerHTML = "";
    productListContainer2.innerHTML = "";
    ...
    fakeProductFetch().then((products) => {
        ...
        // Load first section (Hot Deals)
        hotDeals.forEach((product, index) => {
            setTimeout(() => {
                productListContainer1.innerHTML += createProductCard(product);
            }, index * 150); // Stagger the loading
        });
        
        // Show promotional banner after first section loads
        setTimeout(() => {
            promoBanner.classList.remove("hidden");
            promoBanner.style.display = "block";
        }, hotDeals.length * 150 + 200);
        
        // Load second section (Premium Collection) after banner
        setTimeout(() => {
            premiumCollection.forEach((product, index) => {
                setTimeout(() => {
                    productListContainer2.innerHTML += createProductCard(product);
                }, index * 150);
            });
        }, hotDeals.length * 150 + 800); // Wait for banner to appear
    });
}
